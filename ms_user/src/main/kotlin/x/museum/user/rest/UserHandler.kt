...
import x.museum.user.Router.Companion.apiPath
import x.museum.user.Router.Companion.idPathVar
import x.museum.user.Router.Companion.userPathVar
import x.museum.user.entity.CustomUser
import java.net.URI
import x.museum.user.entity.UserId
import x.museum.user.service.AccessForbiddenException
import x.museum.user.service.CustomUserDetailsService
...
//        val id = UserId.fromString(idStr)
//
...
//        }
//        UserHandler.logger.debug { "findById: $quest" }
//        return toResponse(quest, request)
//    }

    suspend fun findByUsername(request: ServerRequest): ServerResponse {
        println("Handler: findByUsername")
        val userStr = request.pathVariable(userPathVar)
        val username = userStr

        println("USERNAME: $username")

        val user = try {
            service.findByUsernameAndAwait(username) ?: return notFound().buildAndAwait()
        } catch (e: AccessForbiddenException) {
            return status(FORBIDDEN).buildAndAwait()
        }

        UserHandler.logger.debug {  "findByUsername: $user" }
...
//            val msg = e.message ?: ""
//            UserHandler.logger.trace { "InvalidVersionException: $msg" }
//            return status(PRECONDITION_FAILED).bodyValueAndAwait(msg)
//        }
//
//        UserHandler.logger.trace { "Quest updated: $updatedQuest" }
...
//        UserHandler.logger.trace { "Version: $version" }
//
//        if (version.length < 3) {
...
//    }
    ...
    private suspend fun toResponse(user: UserDetails, request: ServerRequest): ServerResponse {
    ...
    private suspend fun getUsername(request: ServerRequest): String {
        ...
        UserHandler.logger.debug { "username = $username" }
        return username
    }
    ...
    private fun getIfMatch(request: ServerRequest): String? {
        ...
        UserHandler.logger.trace { "Version: $versionList" }
    ...
    private fun getIfNoneMatch(request: ServerRequest): String? {
        ...
        UserHandler.logger.debug { "versionHeader: $versionHeader" }
    ...
    fun getBaseUri(headers: HttpHeaders, uri: URI, id: UserId? = null): String {
